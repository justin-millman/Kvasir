...
namespace Kvasir.Translation2 {
    ...
    internal readonly struct Nested<TAttribute> where TAttribute : INestableAnnotation {
        ...
        private Nested(TAttribute annotation, string path) {
            ...
            Debug.Assert(annotation.Path is not null && annotation.Path != CURRENT_PROPERTY);
            Debug.Assert(path is not null);
            Debug.Assert(path == CURRENT_PROPERTY || annotation.Path.EndsWith(PATH_SEPARATOR + path));
